// -------------------------------------------------------------------------------
// Adapted from https://github.com/imiRemy/RK45js
//
...
exports.System.prototype = {
    ...
    checkSetUp: function() {
        if (this.start >= this.stop) {
            ...
            else this.status.message = 'Stop time is less than start time.'
        ...
        if (this.h <= 0) {
            ...
            if (this.h == 0.0)
                ...
                this.status.message =
                    'h is less than zero but must be a positive number.'
            return this.status.state
        }
        return 'ok'
    },
    ...
    computeK: function(t) {
        ...
        for (let i = 0; i < dimension; i++) {
            x_new[i] =
                this.newX[i] +
                (1932.0 * k[i][0] - 7200.0 * k[i][1] + 7296.0 * k[i][2]) /
                    2197.0
        }
        for (let i = 0; i < dimension; i++) {
            k[i][3] = this.h * fn[i]
    ...
    solve: function() {
        ...
        while (t < this.stop) {
            ...
            debug(
                'delta: ' +
                    this.delta +
                    '; min: ' +
                    Math.min.apply(null, this.delta)
            )
            ...
            if (Math.max.apply(null, this.R) <= this.tol) {
                ...
                debug(
                    'Need to adjust step size, max R: ' +
                        Math.max.apply(null, this.R)
                )
                debug('Scale factor: ' + Math.min.apply(null, this.delta))
                this.h = this.h * Math.min.apply(null, this.delta)
            }

            if (this.h >= this.stop - t) this.h = this.stop - t
        ...
        if (this.count++ > this.maxCount) {
            ...
            this.status.state = 'error'
            this.status.message =
                'Iteration count exceeded MAX count (' + this.maxCount + ')'
        } else if (t >= this.stop) {
